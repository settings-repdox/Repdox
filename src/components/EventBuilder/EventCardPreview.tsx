    ...
    <div className="group relative overflow-hidden rounded-xl bg-card border border-border hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5">
      ...
      <div className="relative h-48 overflow-hidden bg-muted">
        {cover ? (
          <img
            src={
              /^blob:|https?:\/\//i.test(cover) ? cover : getEventImage(cover)
            }
          ...
          <div className="absolute top-3 left-3 flex flex-wrap gap-1">
            {Array.isArray(type) ? (
              type.map((t) => (
                <Badge
                  key={t}
                  className="bg-purple-600 hover:bg-purple-700 text-white border-0 text-[10px] px-2 py-0.5"
                >
                  {t}
                </Badge>
              ))
      ...
      <div className="p-5 space-y-3">
        ...
        <p className="text-sm text-muted-foreground line-clamp-2 leading-relaxed">
          {description ||
            "No description yet — add details to make your event stand out."}
        </p>
